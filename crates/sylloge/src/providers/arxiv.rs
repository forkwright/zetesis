...
fn read_feed(body: &[u8]) -> Result<Vec<RawEntry>> {
    ...
    loop {
        ...
        match event {
            Event::Start(start) => feed.open(ns, &start)?,
            Event::Empty(start) => feed.open_empty(ns, &start, reader.decoder())?,
            Event::Text(t) if feed.capturing() => {
                feed.text
                    .push_str(&t.xml10_content().map_err(|e| xml_error(&e))?);
            }
            Event::CData(c) if feed.capturing() => {
                feed.text
                    .push_str(&c.xml10_content().map_err(|e| xml_error(&e))?);
            }
            Event::GeneralRef(r) if feed.capturing() => {
                feed.text.push_str(&resolve_reference(&r)?);
            }
...
struct FeedState {
    ...
    entry: RawEntry,
    /// Text of the open [`Tag::Text`] element.
    text: String,
}

impl FeedState {
    fn capturing(&self) -> bool {
        matches!(self.stack.last(), Some(Tag::Text(_)))
    }

    fn open(&mut self, ns: Ns, start: &BytesStart<'_>) -> Result<()> {
        let tag = self.classify_child(ns, start)?;
        if tag == Tag::Entry {
            self.entry = RawEntry::default();
        }
        self.text.clear();
        self.stack.push(tag);
        Ok(())
    ...
    fn open_empty(&mut self, ns: Ns, start: &BytesStart<'_>, decoder: Decoder) -> Result<()> {
        let tag = self.classify_child(ns, start)?;
        if self.stack.last() == Some(&Tag::Entry) {
            read_attributes(&mut self.entry, ns, start, decoder)?;
        }
...
fn resolve_reference(reference: &BytesRef<'_>) -> Result<String> {
    if let Some(ch) = reference.resolve_char_ref().map_err(|e| xml_error(&e))? {
        return Ok(ch.to_string());
    }
    let name = reference.decode().map_err(|e| xml_error(&e))?;
    quick_xml::escape::resolve_predefined_entity(&name)
        .map(str::to_owned)
        .ok_or_else(|| feed_defect("reference to an undeclared entity"))
}

fn xml_error(error: &dyn std::fmt::Display) -> Error {
