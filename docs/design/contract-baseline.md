...
> and 6 state the contract after that change. Every other cell, and
...
| `Provider` (`provider.rs:51-84`) | Object safety; `search(query: &str, constraints: &SearchConstraints)` signature (`provider.rs:79-83`). `publication_time_capability` defaults to `Unsupported` (`provider.rs:67-69`). *Phase 03 S2:* `query_shapes` defaults to none, so a provider that declares no shape is never routed. `min_request_interval` (default zero) spaces the provider's attempts through the router. `search_with_evidence` (default: `search` with no evidence) returns the evidence envelope fingerprints behind an answer or a failure. | Unique lowercase `name()` (`provider.rs:35-37`); honoring `max_results`, domain lists, language, and freshness; cancellation safety when the future is dropped (`provider.rs:44-50`). `search` receives no consumer identity, ledger handle, attempt identity, or credential reference, so it cannot authorize or charge anything. | Trait implemented; no provider adapter exists. *Phase 03 S2:* `SemanticScholar`, `Arxiv`, and `Wikipedia` implement it over a shared `StaticAcquirer`, each sending only its documented anonymous request |
