...
| Provider search | Requests | count (`ProviderSpend::request_count`, `u32`) | Provider quota | Recorded per call; not enforced. *Phase 03 S2:* recorded per request the provider reports sending; a refused query, a refused pacing slot, or a timed-out attempt records none |
