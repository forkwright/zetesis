...
> attempt receipts, and request builders, parsers, and `Provider`
> implementations over `StaticAcquirer` for Semantic Scholar, arXiv, and
> Wikipedia. Cells marked *Phase 03 S2* in sections 3.1, 3.4, 3.8, 5.2,
> 6, 9, and 10 state the contract after that change. Every other cell, and
...
| `Provider` (`provider.rs:51-84`) | Object safety; `search(query: &str, constraints: &SearchConstraints)` signature (`provider.rs:79-83`). `publication_time_capability` defaults to `Unsupported` (`provider.rs:67-69`). *Phase 03 S2:* `query_shapes` defaults to none, so a provider that declares no shape is never routed. `search` is the one search method and returns a `ProviderAnswer`: the result or error, the evidence envelope fingerprints behind it, and the requests it sent. | Unique lowercase `name()` (`provider.rs:35-37`); honoring `max_results`, domain lists, language, and freshness; cancellation safety when the future is dropped (`provider.rs:44-50`). `search` receives no consumer identity, ledger handle, attempt identity, or credential reference, so it cannot authorize or charge anything. | Trait implemented; no provider adapter exists. *Phase 03 S2:* `SemanticScholar`, `Arxiv`, and `Wikipedia` implement it over a shared `StaticAcquirer`, each sending only its documented anonymous request and holding every caller of the instance to its documented pacing, connection limit, and `Retry-After`. Pacing is the provider's own; the router adds none |
...
| `ResearchResult` (`result.rs:206-304`) | Serializable envelope; `top_hit` ranks NaN last (`result.rs:281-283`). | *Phase 03 S2:* `Router` derives `cache_key` from the whitespace-normalized query, the shape, and the constraints with canonicalized domain lists, and records one attempt receipt per routed provider; `evidence_state` distinguishes answered, no evidence (every asked provider answered cleanly), incomplete (a clean miss beside a provider that failed, timed out, or answered only malformed records), and unanswered. A result built outside the router carries a caller-supplied key and may have empty provenance. No cache layer exists. No schema version. | Implemented as a value |
| `ProvenanceEntry` (`result.rs:306-326`) | *Phase 03 S2:* provider id plus an optional citation and an optional attempt receipt (route ordinal, tier, outcome: answered with drop counts, empty, failed with its error class, timed out, or refused for a paid or unrouted tier or for undated hits under a strict freshness window; and the fingerprints of the evidence envelopes behind the attempt); decoding rejects an entry with neither. | *Phase 03 S2:* no durable attempt identity or endpoint policy revision is recorded. Neither the provider nor the router keeps the envelope or body, so a fingerprint identifies the evidence but cannot retrieve it. | Implemented as a value |
...
| Provider search | Zetesis | `Provider` trait, normalized `ResearchResult`, citations and provenance, cost report, error class | Query, constraints, consumer scope identifier, credential references, explicit paid authorization | Trait implemented; adapters planned. *Phase 03 S2:* Tier-0 adapters for Semantic Scholar, arXiv, and Wikipedia over `StaticAcquirer` |
...
| Provider attempt | Query identity, provider id, endpoint policy revision, attempt ordinal | Durably, before the upstream call | A retry after an unknown outcome reuses the attempt identity | None; provenance and cost carry only a provider id. *Phase 03 S2:* router receipts carry provider id, route ordinal, tier, outcome, and the fingerprints of the evidence envelopes behind the attempt; no durable identity or endpoint policy revision |
...
|---------|------|----------|
| `complete` | Transfer finished within limits and, for document media, extraction finished | Yes |
...
| Provider search | Requests | count (`ProviderSpend::request_count`, `u32`) | Provider quota | Recorded per call; not enforced. *Phase 03 S2:* recorded per request the provider reports sending; a refused query or a refused pacing slot records none; a timed-out attempt records one, since its request may have left |
...
| Provider search | Dropping `search` must not leak partial results or unrecorded spend (`provider.rs:44-50`). Once attempt identity exists, a drop after the request was sent is an unknown outcome; the ledger records it as unknown and a retry reuses the attempt identity. *Phase 03 S2:* the router's per-attempt timeout drops the call and receipts it as timed out, recording one request because it may have left. | Convention only; attempt identity planned |
