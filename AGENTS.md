...
`StaticAcquirer` is the only HTTP client (anonymous static GET with per-hop validation). No provider adapter, cache, durable ledger, or model binding exists. `docs/design/contract-baseline.md` lists what each public type enforces and what is caller convention.
